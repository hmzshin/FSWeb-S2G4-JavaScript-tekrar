...
console.log("Görev1: ", CemberinCevresi(10));
...
enkucuk = KucuguBul(sayilar);

function BuyuguBul(arr) {
  let enBuyuk = arr[0];
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] > enBuyuk) {
      enBuyuk = arr[i];
...
//3c çözümü:
ucebolunenlerintoplami = ucetambolunenler.reduce((toplam, number) => {
...
for (let i = 0; i < sayilar.length; i++) {
  const filtre = sayilar.filter((number) => number === sayilar[i]);
  if (filtre.length > 1) {
    info.push({ number: sayilar[i], repetition: filtre.length });
  }
}

tekraredensayilar = info.map(
  (obje) => `${obje.number} sayısı ${obje.repetition} kere tekrar edilmiştir`
);
